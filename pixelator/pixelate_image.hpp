...
// // the pixelator::Image object of the same size as the input image with all
// the
// // pixels having the same values as the original image.

// // static_cast<float>(number) converts (aka "casts") a number to float.
// const auto factor_cols = smaller_size.cols /
// static_cast<float>(image.cols()); const auto factor_rows = smaller_size.rows
// / static_cast<float>(image.rows()); const auto smallest_factor =
// std::min(factor_cols, factor_rows);
